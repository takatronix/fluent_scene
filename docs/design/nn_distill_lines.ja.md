...
## 9. 第2ラウンド追記 (2026-08-20): 濃さ修正 と tiny_ink

**§5の受け入れ基準1「目視同等」は判定が甘かった**。COCO検証画像では気に
ならなかったが、実写のwebカメラで教師と並べると生徒の線が明らかに薄い
(教師のインク画素で 教師0.369 vs 生徒0.561)。真因は線が画素の約4%しかなく
BCEが紙側に支配されること。`--ink-weight 10` (線画素へのBCE重み) で解決 —
較正で 0/4/10 を比較し決定。今後この種のモデルでは既定にすべき。

**tiny_ink (太いイラスト調) を追加**。オーナーが気に入った portrait sketch
は AnimeGANv3 = 非商用なので、§2どおり教師にせず、教師を自作した
(`nn/distill/ink_target.py`)。鍵は「教師に入れる前に写真を平滑化する」こと
— 質感が無ければ教師は線を引かないので、イラスト的な単純化が無料で得られる。
2値ターゲットに対する生徒のグレーは不確実性なので、`--harden 3` で
ONNXグラフに硬化カーブを焼き込む (中間グレー 27%→8%、目標8.3%)。

**罠**: 線の太さも平滑化半径もピクセル固定 = 画風が解像度依存。tiny_ink は
512²前後で使う (anime.html が自動で合わせる)。1024²では細い線画になる。

失敗して捨てた案 (2値太線を直接学習、輝度によるベタ塗り) は
nn/distill/runs/NOTES.md に理由付きで記録。
