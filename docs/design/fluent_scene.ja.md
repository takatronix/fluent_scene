# fluent_scene 設計書 — Stage / Layer / Surface と Scene の関係
...
> **2026-08-14 追記(ライブラリ名)**: リポジトリ/ライブラリ名は **fluent_scene**
> に統一(オーナー決定)。製品の顔はユーザーと AI が書く Scene 文書であり、
> Stage は実行層のクラス名として残る(React と Fiber の関係)。名前空間は
> `fluent_scene`(実行層)+ `fluent_scene::fvs`(文書層)。

#### 先行 OSS の検討(GPUPixel / GPUImage 系)

フィルタエンジンとして GPUPixel 等の採用を検討したが**不採用**。理由:
それらは OpenGL(ES) のフィルタパイプライン単体であり、本設計の核である
①CPU/GPU 同一出力の golden 契約(AI の検証ループの土台) ②CPU リファレンス
(GPU なし環境・WASM への道) ③宣言文書+digest+リンター ④レイヤー合成・
テキスト・UI を持たない。フィルタ本体は GLSL∩C++ 単一ソースで 1 ファイルに
収まる規模で、導入の節約が小さい一方、GL 依存(本機は Vulkan)と決定性の
欠如という代償が大きい。
...
5. 初期カタログ: `ui.button`（momentary）/ `ui.switch`（トグル）/
   ...
   **実装状況（2026-08-14）**: §10-3 のポインタ注入と、button / switch /
   ...
   （`fluent_scene/ui.hpp`）。segmented と dropdown は同日のオーナー指示で
