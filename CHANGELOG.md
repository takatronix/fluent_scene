...
- **pencil lines (MIT)**: Informative Drawings (Chan+ 2022, **MIT**) の
  線画生成器を第3スタイルとして追加。繊細な鉛筆画で PortraitSketch (ink)
  と好対照。動的入力サイズ対応なので全画面でもクロップでも使える。
  IO は AnimeGAN と別形 (NCHW・0..1・1ch出力) — modelKind で分岐
- オーナー発案「NN の線を anime/sumie フィルタに合成」のライセンス
  クリア解: これが商用可の線ソース第1号 (AnimeGANv3 系は非商用)。
  1ch 線強度マップなのでレイヤーマスク/乗算合成の素材にそのまま使える
- 線の太さ=推論解像度ノブの実証 (320=豪快な筆/512=中庸/800=ペン画)。
  face lock は自動判定に戻し、線画モデルは全画面既定
- fp16 版 8.6MB (mean|Δ|3e-4)。?m= で URL からスタイル指定可

## 0.14.6 — 2026-08-20 (anime.html: 顔特化に整理 + fp16 + GAP→ReduceMean)

オーナー判断「はやおやしんかいはいらん、顔だけでいい」に従い整理:

- Hayao/Shinkai (風景用) をピッカーと同梱モデルから削除。スコープ選択
  (人物だけ/背景だけ) とセグメンテーション経路も撤去 — 顔ロック特化
- **fp16版を追加し WebGPU EP はそちらを取得** (JP_face 6.1→3.1MB、
  Sketch 4.2→2.2MB。CPU EP は fp32 のまま)。fp16 化の罠2つ:
  ①無名ノードがあると変換器の cast テンソル名が衝突して壊れたグラフに
  なる (変換前に全ノードへ一意名を付与) ②画質は mean|Δ|≈2e-3 で目視差なし
- **GAP→ReduceMean 等価置換** (axes=[2,3] keepdims): JP_face だけ速い
  残存差が ReduceMean 型正規化だったため、全モデルの
  GlobalAveragePool を数学的等価の ReduceMean に置換 (max|Δ|≤2e-5)
- 解像度ラダー表記を「512px (native)」に — モデルの学習解像度が512で、
  それ未満のクロップは学習分布外で崩れる (オーナー実測と一致)

## 0.14.5 — 2026-08-20 (anime.html: 顔ロック — 「のっぺらぼう」の真犯人は実装だった)

オーナー評「実装が悪いだけだと思うよ」が正解だった回。モデルの限界と
誤診したことへの訂正:

- **真犯人1 = 前処理の欠落**: 綺麗な AnimeGAN デモは全て顔を検出・
  クロップして学習スケールでモデルに入れている。引きのフレームを丸ごと
  縮小して流すと顔が学習分布外の小ささになり、のっぺらぼう化する。
  同一重み・同一画像で「丸ごと=のっぺらぼう / クロップ=綺麗なアニメ顔」
  を CPU 実証してから実装
- **顔ロック実装**: MediaPipe FaceDetector (Apache-2.0) → EMA+デッドバンド
  の正方形クロップ (マージン2.1倍) → モデル解像度で推論 → フェザー付き
  貼り戻し (時刻一致ペアの原則は維持)。顔モデル (JP_face/PortraitSketch)
  で自動ON、👤ボタンで手動切替。顔ロスト1.5秒でフルフレームへ復帰
- **真犯人2 = to8() の256床がアスペクト破壊**: 256/384段で 16:9 が
  正方形に潰れていた (256px段は 256×256)。短辺基準で両辺スケールに修正
  (256段=456×256)。既定解像度を512に
- 既定モデルの品質評価は必ず「実運用の構図」で行うこと — 風景サンプル
  だけ見て顔モデルを出荷したのが今回の敗因
